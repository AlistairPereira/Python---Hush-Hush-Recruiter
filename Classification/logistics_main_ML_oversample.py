...
# Set encoding for stdout ,to avoid issues with special characters.
...
xtrain_sampled, ytrain_sampled = smote.fit_resample(xtrain_scaled, ytrain)
...
# Train the model
log_reg = LogisticRegression(max_iter=2, solver='lbfgs')
log_reg.fit(xtrain_sampled, ytrain_sampled)

# Predict on the original test data
ypred = log_reg.predict(xtest_scaled)
ypred_proba = log_reg.predict_proba(xtest_scaled)

with open('logreg_model_.pkl','wb') as model_file:
...
# ac_ypred = accuracy_score(ytest,ypred)
# print("ac_ypred",ac_ypred)
# Evaluate the model
print("\nConfusion Matrix:")
