...
# Filter to include only predicted_label = 1




# Optionally, store the filtered top candidates into a new table
# def store_filtered_candidates_to_db(df):
#     conn = sqlite3.connect('stackoverflow_users.db')
#     df.to_sql('top_cand', conn, if_exists='replace', index=False)
#     conn.close()
